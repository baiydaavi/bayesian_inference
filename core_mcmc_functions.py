...
def chain(data, max_trials=10000, convergence_window=50, convergence_threshhold=.001,
    ...
    if variances == 'systematic':
        covariance = np.array([
            ...
            [-3.856e-4, 1.005e-2, 1.000e+0, 2.662e-2],
...
def likelihood_test(resolution, p1_min=.1, p1_max=1.2, p2_min=.5, p2_max=1.15,
    ...
    ax[0].plot(p1, p1_lik)
    ax[0].set_title('log likelihood of ' + names[0] + ' when ' + names[1] + ' ={}'.format(p2_slice))
    ax[1].plot(p2, p2_lik)
    ax[1].set_title('log likelihood of ' + names[1] + ' when ' + names[0] + '={}'.format(p1_slice))
